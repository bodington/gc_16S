...
#rownames(otu_table(ps_raw)) <- replace(rownames(otu_table(ps_raw)), c(31, 42), c("LD_0", "CM_0"))
...
#  tax_filter(min_sample_abundance = 1)
```
...
phyloseq_write_dataset(ps_16S, "./01_data/06_tree/", "ps_16S")
...
asv_tree <- read_tree("01_data/06_tree/ps_16S_ASVs.tre")
...
unifrac_plot <- plot_ordination(ps,
                                  ...
                                  color="soil")
# geom_label(
#   label = sample_data(ps)$name
#    nudge_x=0.1,
#    nudge_y=0.1
# )


ggplot2::ggsave(unifrac_plot, file = "./02_out/test_unifrac.png", height = 12, width = 12)
```

```{r}
ps <- ps_16S

bray = ordinate(ps, "NMDS", "bray")
bray_plot <- plot_ordination(ps,
                                  bray,
                                  "samples",
                                  color="soil")
# geom_label(
...
ggplot2::ggsave(bray_plot, file = "./02_out/test_bray.png", height = 12, width = 12)
```
...
```{r fractions_relabund, fig.height=6, fig.width=20}
ps <- ps_merge %>%
  subset_samples(!sample %in% c("4.5_20", "4.5_M5", "4.5_M11", "7.5_20", "7.5_M4", "7.5_M14", "CM_20", "CM_M5", "CM_M11", "LD_20", "LD_M4", "LD_M13")) %>%
...
frac_taxa_plot$data$top[frac_taxa_plot$data$top %in% c("other", NA)] <- rep("Other", length(frac_taxa_plot$data$top[frac_taxa_plot$data$top %in% c("other", NA)]))
...
```{r mags_relabund, fig.height=6, fig.width=20}
ps <- ps_merge %>%
  subset_samples(sample %in% c("4.5_M5", "4.5_M11", "7.5_M4", "7.5_M14", "CM_M5", "CM_M11", "LD_M4", "LD_M13")) %>%
  filter_taxa2(~ sum(. > 0) > 0) %>%
  mutate_tax_table(across(everything(), \(x) str_replace(x,'_[A-Z]', ''))) %>%
  subset_taxa(Kingdom != "")
...
my_pal <- c(pal_simpsons("springfield")(16)[c(13,7)])

mag_taxa_plot <- plot_bar(ps,
                          x = "label", 
                          fill="top") +
#  facet_wrap(~soil,
#             ncol = 4,
...
mag_taxa_plot$data$top[mag_taxa_plot$data$top %in% c("other", NA)] <- rep("Other", length(mag_taxa_plot$data$top[mag_taxa_plot$data$top %in% c("other", NA)]))
mag_taxa_plot$data$top <- str_pad(mag_taxa_plot$data$top, 21, "right")
domain_levels <- str_pad(domain_levels, 21, "right")
mag_taxa_plot$data$top <- factor(mag_taxa_plot$data$top, levels = domain_levels)
mag_taxa_plot$layers[[1]]$geom_params$width <- 1

ggsave(mag_taxa_plot, file = "./02_out/test_mag_relabund.png", height = 6, width = 6)
```
