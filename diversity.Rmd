...
library(metagMisc)
library(Biostrings)
source_url("https://raw.githubusercontent.com/CGR-lab/barbecue/main/scripts/R/phyloseq.mate.R")
```
...
sample_data(ps_merge)$soil <- factor(sample_data(ps_merge)$soil, levels = c("GcA (Craibstone pH 4.5)", "GcB (Craibstone pH 4.5)", "GcC (Corsemaul)", "GcL (Lower Dell)"))
sample_data(ps_merge)$label <- factor(sample_data(ps_merge)$label, levels = sample_data(ps_merge)$label)

ps_all <- subset_samples(ps_merge, !label %in% c("Fill1", "Fill2", "Fill3", "Fill4", "Fill5", "Fill6", "Fill7", "Fill8"))
...
set.seed(90210)
ps_all_rare <- rarefy_even_depth(ps_all, sample.size = 15000)

alpha_all <- alpha(ps_all_rare, index = "all")
...
#  tax_transform(trans = "compositional", rank = "Phylum") %>%
  ...
  aggregate_rare("Phylum", detection = 0.3, prevalence = 0.3)
...
zp2 <- observed_plot +
  scale_y_continuous(limits = c(180, 1200),
                     breaks = c(500, 1000)) +
  labs(y = "Richness") +
  theme(
    title = element_blank(),
...
ggplot2::ggsave(zp, file = "./02_out/02_phyla_diversity.png", height = 8, width = 14)
```
