...
#sample_data(ps_16S)$label <- read.csv(file = "00_ref/phyloseq_metadata.csv")[, c(13)]
```
...
write.table(tax_glom(ps_merge_proteo, taxrank = "Phylum", bad_empty = "Bacteria_unidentified") %>%
              psmelt() %>%
              select(Phylum, Sample, Abundance) %>%
              spread(Sample, Abundance),
            file = "./02_out/ps_merge.relative_abundance.phylum.csv",
            sep = ",",
            quote = F,
            row.names = F,
            col.names = T
...
zp <- ggarrange(zp1, zp2,
          ...
          align = "v")
zp
ggplot2::ggsave(zp, file = "./02_out/01_alpha_diversity.png", height = 6, width = 14)
```
