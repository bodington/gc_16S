...
sample_data(ps_16S)$shannon <- estimate_richness(ps_16S, measures = c("Shannon"))
...
write.table(tax_glom(ps_16S_proteo, taxrank = "Phylum", bad_empty = "Bacteria_unidentified") %>%
            ...
            file = "./02_output/ps_16S.relative_abundance.phylum.csv",
            sep = ",",
            quote = F,
            row.names = F,
            col.names = T
...
ps <- ps_16S_proteo %>%
  ...
  aggregate_rare("Phylum", detection = 0.05, prevalence = 0.1) %>%
  transform_sample_counts(function(x) x * 100)

taxa_16S_plot <- plot_bar(ps,
                          x = "name", 
                          fill="Phylum",
                          title = "Community Structure of Soil Community Fractions") +
  facet_wrap(~soil,
             ncol = 4,
             scales="free_x") +
  labs(fill = "Phylum",
...
```{r pub_fig_1, fig.height=12, fig.width=12}
